...
class TransformG
{
  public:
    ...
    template <typename PointTT> void transformPointInPlace( PointTT & in)
    {
      boost::numeric::ublas::matrix<double> matIn(4, 1);
...
};// end of class tranformG


tf::Quaternion  getQuaternion(geometry_msgs::Quaternion q)
{
  return tf::Quaternion(q.x, q.y, q.z, q.w);
}

tf::Vector3 getVector3(geometry_msgs::Vector3 v)
{
...
TransformG readTranform(const string & file)
{
  ...
  BOOST_FOREACH(rosbag::MessageInstance const mtf, view_tf) {
    ...
    assert(tf_ptr != NULL);
    std::vector<geometry_msgs::TransformStamped> bt = tf_ptr->transforms;

...
vector<pcl::PointCloud<PointT>, Eigen::aligned_allocator<pcl::PointCloud<PointT> > > cloudVector;
std::vector<std::map<int, int> > segIndex2LabelVector;
vector<vector<SpectralProfile, Eigen::aligned_allocator<SpectralProfile> > >  spectralProfilesVector;
vector<PointCloudVector > segment_cloudsVector;
...
vector<vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > > locations;
vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > maximas(NUM_CLASSES);
...
void createTrees(const PointCloudVector &segment_clouds, std::vector< pcl::KdTreeFLANN<PointT>::Ptr > &trees) {
...
int findNeighbors(PointT centroid, const PointCloudVector &segment_clouds, const std::vector< pcl::KdTreeFLANN<PointT>::Ptr > &trees, vector <int> &neighbor_map) {
    neighbor_map.clear();
    float tolerance = 0.6;
 ...
 */
int get_neighbors(const PointCloudVector &segment_clouds, map< pair <int, int>, float > &distance_matrix, map <int, vector <int> > &neighbor_map) {
    float tolerance = 0.6;
    // get distance matrix
    for (size_t i = 0; i < segment_clouds.size(); i++) {
...
{
  compute3DCentroid(cloud, spectralProfile.centMatrix);
  spectralProfile.centroid.x=spectralProfile.centMatrix[0];
  spectralProfile.centroid.y=spectralProfile.centMatrix[1];
  spectralProfile.centroid.z=spectralProfile.centMatrix[2];
}
...
void get_pair_features(int segment_id, vector<int> &neighbor_list,
        ...
        vector<SpectralProfile, Eigen::aligned_allocator<SpectralProfile> > & spectralProfiles,
...
void parseAndApplyLabels(std::ifstream & file, pcl::PointCloud<pcl::PointXYZRGBCamSL> & cloud, PointCloudVector & segment_clouds, map<int, int> & segIndex2label) {
    string tokens[3];
...
void lookForClassInOriginalFrameOrthographic(vector<int> & classes, pcl::PointCloud<pcl::PointXYZRGBCamSL> & cloud, vector<SpectralProfile, Eigen::aligned_allocator<SpectralProfile> > & spectralProfiles, map<int, int> & segIndex2label, const PointCloudVector &segment_clouds, int scene_num, vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > & maximas)
{
...
void lookForClassInOriginalFrameProjective(vector<int> & classes, pcl::PointCloud<pcl::PointXYZRGBCamSL> & cloud, vector<SpectralProfile, Eigen::aligned_allocator<SpectralProfile> > & spectralProfiles, map<int, int> & segIndex2label, const PointCloudVector &segment_clouds, int scene_num, vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > & maximas)
{
    pcl::PointXYZ steps(0.01, 0.01, 0.01);
    std::vector< pcl::KdTreeFLANN<PointT>::Ptr > trees;
...
void lookForClass(vector<int> & classes, pcl::PointCloud<pcl::PointXYZRGBCamSL> & cloud, vector<SpectralProfile, Eigen::aligned_allocator<SpectralProfile> > & spectralProfiles, map<int, int> & segIndex2label, const PointCloudVector &segment_clouds, int scene_num, vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > & maximas)
{
...
int write_feats(TransformG transG, pcl::PointCloud<pcl::PointXYZRGBCamSL>::Ptr & cloud_ptr, int scene_num) {
    ...
    filterInPlace(cloud_ptr);
    segmentInPlace(cloud);
    ...
    for (int seg = 1; seg <= max_segment_num; seg++) {
        ...
        if (!cloud_seg->points.empty() && cloud_seg->points.size() > MIN_SEG_SIZE) {
            ...
            pcl::PointXYZRGBCamSL tmp_point = cloud_seg->points[1];
...
void getMovement(bool lookFor)
{
    ...
    if (lookFor) {
        ...
        while (cloudVector.size() > 0) {
            vector<pcl::PointXYZI, Eigen::aligned_allocator<pcl::PointXYZI> > frame_maximas;
...
void evaluate(string inp, string out, int oclass)
{
    ...
    vector<PointXYZI, Eigen::aligned_allocator<PointXYZI> > maximas;
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber cloud_sub_ = n.subscribe("/camera/depth_registered/points", 1, processPointCloud);
